...
public class PageDTO {
	...
	public PageDTO(int total, Criteria criteria) {
		...
		// 올림처리, 페이지에 게슬기 한 개라도 있다면, 올림을 하여 페이지를 표시하기 위함.
		this.endPage = (int)Math.ceil(criteria.getPageNum() / (double)criteria.getAmount()) * criteria.getAmount();
		// endPage에서 - 9 를 하면 startPage를 구할 수 있다.
		this.startPage = endPage - (criteria.getAmount() - 1);
		// 실제 마지막 페이지는 전체 게시글 개수에서 amount를 나눠주게 한다. 하지만 1개의 게시글이 있더라도
		// 페이지를 표현해야 하기 때문에 실수로 나누어 준 다음 소수점을 올림하여 페이지를 +1 해준다.
